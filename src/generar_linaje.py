...
Este artefacto no pretende sustituir el catálogo/lineage institucional de CGR;
documenta de forma reproducible el recorrido implementado en el repositorio.
"""
...
LINAJE = [
    ...
    ("favoritismo", "SEACE/OCDS o sintético", "monto", "imputación de nulos; agregación por proveedor-entidad", "monto", "monto_total / monto_promedio", "sklearn RF / Spark MLlib RF", "ranking_riesgo_favoritismo.score_riesgo_favoritismo", "El score es señal de priorización, no hallazgo."),
    ...
    ("fraccionamiento", "SEACE/OCDS o sintético", "monto", "suma dentro de ventana y grupo", "monto", "monto_total_ventana_15d / monto_total_grupo", "sklearn IsolationForest / Spark MLlib KMeans", "ranking_riesgo_fraccionamiento.score_anomalia", "Score de anomalía relativo."),
    ...
    ("arquitectura", "data/*.csv", "fuentes sintéticas", "copia sin transformación", "lakehouse/bronce/*", "N/A", "ETL local PoC", "N/A", "Bronce simula la capa raw del TDR."),
    ("arquitectura", "lakehouse/bronce/*", "contratos/dimensiones", "limpieza, imputación y feature engineering", "lakehouse/plata/*", "datasets de modelado", "ETL local PoC", "N/A", "Los modelos canónicos consumen Plata."),
    ("arquitectura", "lakehouse/plata/*", "features/dimensiones", "scoring/modelado", "N/A", "scores y señales", "sklearn + Spark MLlib/GraphFrames", "lakehouse/oro/ranking_*.csv", "Oro contiene solo salidas downstream; no datasets intermedios."),
]
