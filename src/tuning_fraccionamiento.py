...
PARAM_GRID = {
    ...
    # contamination no cambia el orden del score de anomalía, pero se incluye
    # para verificar estabilidad del clasificador binario interno.
    "contamination": ["auto", 0.03, 0.05, 0.08],
}

FINAL_TEST_SIZE = 0.375  # con 8 positivos deja ~3 en test y ~5 en desarrollo
VALIDATION_SIZE = 0.40
VALIDATION_SEEDS = [11, 23, 37, 53, 71]


def recall_at_k(modelo, scaler, X_eval, y_eval):
    """Recall@K, con K igual al número de positivos conocidos en evaluación."""
    ...
    return float(top["label"].sum() / n_pos)
...
def evaluar_configuracion_en_desarrollo(desarrollo, params):
    ...
    for seed in VALIDATION_SEEDS:
        y_dev = desarrollo["label_fraccionamiento_real"].astype(int)
...
def evaluar_holdout_final(desarrollo, test, mejor):
    ...
    recall = recall_at_k(
        ...
        scaler,
        test[FEATURES],
        test["label_fraccionamiento_real"].astype(int),
    )
    return recall
...
def main():
    ...
    print(
        f"Dataset total: {len(df)} grupos / {int(df['label_fraccionamiento_real'].sum())} positivos.\n"
        f"Desarrollo: {len(desarrollo)} / {int(desarrollo['label_fraccionamiento_real'].sum())} positivos.\n"
        f"Holdout final (intocable durante tuning): {len(test)} / "
        f"{int(test['label_fraccionamiento_real'].sum())} positivos."
    )

    mejor, resultados = seleccionar_mejor(desarrollo)
    recall_test = evaluar_holdout_final(desarrollo, test, mejor)

    resumen = {
        "diseno": "holdout final separado antes del tuning; tuning con validaciones repetidas en desarrollo",
        "n_total": len(df),
    ...
    print("\nMejor configuración según validación interna:")
    print(resumen["mejor_configuracion"])
    print(f"Recall@K en holdout FINAL: {recall_test}")
    print("El holdout final no participó en la selección de hiperparámetros.")
    return resumen
